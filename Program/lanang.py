...
class KarakterLanang:
    ...
    def __init__(self):
        ...
        self.lanang_duck = False
        self.lanang_run = True
        self.lanang_jump = False
    ...
    def update(self, userInput):
        if (userInput[pygame.K_UP] or userInput[pygame.K_SPACE]) and not self.lanang_jump:
            self.lanang_duck = False
            self.lanang_run = False
            self.lanang_jump = True
            self.k_jump_sound.play()
        elif userInput[pygame.K_DOWN] and not self.lanang_jump:
            self.lanang_duck = True
            self.lanang_run = False
            self.lanang_jump = False
            self.k_duck_sound.play()
            self.k_duck_sound.set_volume(0.4)
        elif not (self.lanang_jump or userInput[pygame.K_DOWN]):
            self.lanang_duck = False
            self.lanang_run = True
            self.lanang_jump = False
        ...
        if self.lanang_duck:
            self.duck()
        elif self.lanang_run:
            self.run()
        elif self.lanang_jump:
            self.jump()
...
def main():
    ...
    while run:
        ...
        lanang.draw_hitbox(SCREEN)
        lanang.update(userInput)
        ...
        for obstacle in obstacles:
            ...
            obstacle.update()
            if lanang.k_rect.colliderect(obstacle.rect):
...
if __name__ == "__main__":
    print("Gabisa di file lanang.py, run di main.py")
